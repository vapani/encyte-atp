#!/usr/bin/env python3
"""Regenerate the contract template from a source .docx by replacing values with tokens.

This ran once, against the original NGA document, and produced
template/atp-website.docx. It is kept for the case where the template has to be
rebuilt from a fresh source document - not for routine use, because the template
has been edited many times since and re-running this would discard all of it.

    python3 build/tokenise.py <source.docx> <output.docx> [--force]

It refuses to write over the live template unless forced, and it does nothing on
import: an earlier version did its work at module level, so importing it to
borrow a helper silently overwrote the template.
"""
import copy, os, re, shutil, sys

import docx

USAGE = "usage: tokenise.py <source.docx> <output.docx> [--force]"
LIVE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                    "template", "atp-website.docx")
...
def tokenise(src, dst):
    """Copy `src` to `dst`, then replace its values with tokens in place."""
    shutil.copy(src, dst)
    d = docx.Document(dst)
...
def main(argv):
    ...
    if not os.path.exists(src):
        sys.exit("no such source document: " + src)
    if os.path.abspath(dst) == os.path.abspath(LIVE) and not force:
        sys.exit("refusing to overwrite the live template.\n"
                 "  It has been edited many times since it was generated, and this would\n"
                 "  discard every one of those changes. Write somewhere else, or pass\n"
